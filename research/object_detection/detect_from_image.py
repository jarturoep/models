...
def run_inference(model, category_index, image_path):
    if os.path.isdir(image_path):
        ...
        for i_path in image_paths:
            ...
            found_products = {}
            object_count = 0
            print(enumerate(output_dict['detection_boxes']))
            for x, (y_min, x_min, y_max, x_max) in enumerate(output_dict['detection_boxes']):
                if output_dict['detection_scores'][x] > threshold:
                    ...
                    if current_label in found_products:
                        found_products[current_label] += 1
                    else:
                        found_products[current_label] = 1
                    print(y_min)
                    print(x_min)
                    ...
                    print(x_max)
                    object_count = +1
                print(f'Total Object found in {i_path}: {object_count}')
...
#  python .\detect_from_image.py -m .\inference_graph\saved_model -l .\labelmap.pbtxt -i .\test_images
